      ...
      <div className="bg-[#0A0A0A] border border-white/5 rounded-xl shadow-sm flex flex-col">
        ...
        <div className="p-3 px-4 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input type="text" placeholder="Search tv shows..." className="bg-[#000000] border border-white/5 rounded-md pl-9 pr-4 py-2 text-sm focus:outline-none focus:border-red-500 text-white w-64 transition-colors" />
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-gray-400 text-xs font-semibold border-b border-white/5">
                <th className="py-2.5 px-4 cursor-pointer hover:text-white transition-colors w-full">
                  <div className="flex items-center gap-2">TV Show <SortIcon /></div>
                </th>
                ...
                <th className="py-2.5 px-4 cursor-pointer hover:text-white transition-colors whitespace-nowrap">
                  <div className="flex items-center gap-2">Air Date <SortIcon /></div>
                </th>
                <th className="py-2.5 px-4 cursor-pointer hover:text-white transition-colors whitespace-nowrap">
            ...
            <tbody className="text-sm text-gray-300">
              {tvShows.length > 0 ? tvShows.map((show: any) => (
                <tr key={show.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="py-2.5 px-4 whitespace-nowrap">
                    <span className="bg-white/5 text-gray-300 px-2 py-0.5 rounded text-[11px] font-medium">
                      {show.genres && show.genres.length > 0 ? show.genres[0].name : 'Uncategorized'}
                    </span>
                  </td>
                  ...
                  <td className="py-2.5 px-4 text-gray-400 text-[12px] whitespace-nowrap">
                    {show.views > 0 ? show.views.toLocaleString() : '-'}
                  </td>

                  <td className="py-2.5 px-4 text-right">
                    <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/admin/tv-shows/${show.id}`} className="text-white/50 hover:text-white px-2.5 py-1 rounded border border-white/5 hover:bg-white/10 transition-colors text-[11px] font-medium">Edit</Link>
                ...
                <tr>
                  <td colSpan={6} className="py-8 text-center text-gray-500 text-sm">No TV shows found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="px-4 py-2 flex items-center justify-between text-xs text-gray-400">
          ...
          <div className="flex items-center gap-6">
            <span>1 - {Math.min(10, tvShows.length)} of {tvShows.length}</span>
            <div className="flex items-center gap-1">
              ...
              {tvShows.length > 10 && <button className="w-7 h-7 flex items-center justify-center rounded hover:bg-white/5 transition-colors">2</button>}
              <button className="w-7 h-7 flex items-center justify-center rounded hover:bg-white/5 transition-colors disabled:opacity-50" disabled={tvShows.length <= 10}>&gt;</button>
            </div>
          </div>
        </div>
