...
interface MediaPayload {
  video?: (StreamableVideo & { metadata?: { content_type?: string } | null })[];
}

async function getMovie(slug: string): Promise<MovieSummary | null> {
...
async function getMovieMedia(slug: string): Promise<MediaPayload | null> {
...
export default async function WatchMoviePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [movie, media] = await Promise.all([getMovie(slug), getMovieMedia(slug)]);
  ...
  return (
    <div className="bg-black min-h-screen w-full">
      <VideoPlayer src={streamUrl} title={movie.title ?? ''} poster={posterUrl} />
    </div>
  );
}
