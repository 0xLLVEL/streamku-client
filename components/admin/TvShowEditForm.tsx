...
export function TvShowEditForm({ tvShow }: { tvShow?: any }) {
  ...
  const [deletingSeason, setDeletingSeason] = useState<number | string | null>(
    null,
  );
  ...
  const handleDeleteSeason = async (
    ...
    setDeletingSeason(seasonNumber);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!targetId && previewData?.tmdb_id) {
      ...
      if (!importRes.success) {
        setMessage({
          text: importRes.error || "Failed to import TMDB data",
          type: "error",
        });
        setIsSaving(false);
        return;
    ...
    >
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-white/10 bg-[#050505]/80 backdrop-blur-md z-10 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-medium text-white tracking-tight">
            {tvShow ? `Edit "${tvShow.name}"` : "Create TV Show"}
          </h1>
          {tvShow && (
            <a
              href={`/tv-shows/${tvShow.slug}`}
              target="_blank"
              rel="noreferrer"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
                  ...
                  <div className="flex flex-row items-center gap-5 relative z-10">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-white/50 max-w-sm leading-relaxed">
                        ...
                        seasons, posters, backdrops, and cast.
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-col items-stretch gap-4 w-full md:w-[50%] relative z-10">
                    ...
                    <div className="relative" ref={dropdownRef}>
                      ...
                      <input
                        ref={inputRef}
                        type="text"
                        value={isImporting ? "Importing data, please wait..." : searchQuery}
                        ...
                        className="w-full bg-[#000000] border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-white text-sm focus:border-red-600 focus:ring-1 focus:ring-red-600/20 focus:outline-none transition-all placeholder:text-white/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:animate-pulse"
                      />
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    name="original_name"
                    defaultValue={
                      displayData?.original_name || displayData?.name || ""
                    }
                  />
                </div>
                  ...
                  <Input
                    ...
                    defaultValue={
                      displayData?.first_air_date
                        ? displayData.first_air_date.split("T")[0]
                        : ""
                    }
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    defaultValue={displayData?.number_of_seasons || 0}
                    readOnly
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Select
                    name="status"
                    defaultValue={displayData?.status || "Returning Series"}
                  >
          ...
          {/* SEASONS TAB */}
          <div className={activeTab === "seasons" ? "block" : "hidden"}>
            <div className="max-w-4xl space-y-8 animate-in fade-in duration-300">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-white">
                  Seasons ({displayData?.seasons?.length || 0})
                </h2>
                {tvShow?.id && (
                  <Link
                    href={`/admin/tv-shows/${tvShow.id}/seasons/create`}
                    className="bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-md text-xs font-semibold transition-colors flex items-center gap-1.5 border border-white/5"
                  >
                    <svg
                      className="w-3.5 h-3.5"
              ...
              <div className="bg-[#050505] rounded-xl border border-white/5 overflow-hidden divide-y divide-white/5">
                {displayData?.seasons?.map((season: any) => {
                  const content = (
                    <>
                      ...
                      <div className="flex-1 min-w-0 flex items-center justify-between gap-6">
                        <div className="flex-1 min-w-0">
                          <p
                            className={`font-medium text-sm text-white truncate transition-colors ${tvShow?.id ? "group-hover:text-red-400" : ""}`}
                            title={season.name}
                          >
                            {season.name}
                          </p>
                        ...
                        <div className="flex items-center gap-4 text-xs text-white/40 shrink-0">
                          {season.air_date && (
                            <span>{season.air_date.split("T")[0]}</span>
                          )}
                        </div>
                      </div>
                      {tvShow?.id && (
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            type="button"
                            onClick={(e) =>
                              handleDeleteSeason(e, season.season_number)
                            }
                            disabled={deletingSeason === season.season_number}
                            className="p-1.5 text-white/20 hover:text-red-500 hover:bg-red-500/10 rounded transition-all z-10 disabled:opacity-50"
                            title="Delete season"
                          >
                            {deletingSeason === season.season_number ? (
                              <svg
                  ...
                  return tvShow?.id ? (
                    <Link
                      href={`/admin/tv-shows/${tvShow.id}/seasons/${season.season_number}`}
                      key={season.id}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {displayData?.images?.backdrops
                    ...
                    .map((img: any, i: number) => (
                      <div
                        key={i}
                        className="aspect-video bg-[#050505] rounded-xl border border-white/5 overflow-hidden relative group cursor-pointer"
                        onClick={() =>
                          setPreviewImage(
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                  {displayData?.images?.posters
                    ...
                    .map((img: any, i: number) => (
                      <div
                        key={i}
                        className="aspect-[2/3] bg-[#050505] rounded-xl border border-white/5 overflow-hidden relative group cursor-pointer"
                        onClick={() =>
                          setPreviewImage(
                            `https://image.tmdb.org/t/p/original${img.file_path}`,
                          )
